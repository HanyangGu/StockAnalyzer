...
**短期时机覆盖（v0.5b新增）：** 当所有维度满足Strong Buy但 `short_term < 40` 时，标题降为Cautious Buy，行动建议改为"等待回调后入场"。
...
*报告基于 v0.5b 版本，所有权重数据均从 `core/weights.py` 直接提取，所有评分逻辑均从对应 scorer 文件提取，确保与代码完全同步。*
