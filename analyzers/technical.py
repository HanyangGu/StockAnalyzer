...
def compute_bollinger_bands(closes: pd.Series,
    ...
    # BB Width: band width as % of mid price.
    # width_ratio compares current width to its 20-day rolling average:
    #   < 0.75 = squeeze (contraction) -- breakout likely approaching
    #   > 1.25 = expansion -- volatility already elevated
    bb_width_pct = round(bb_range / mid * 100, 2) if mid > 0 else 0.0
    widths_series  = (ma + BB_STD * std) - (ma - BB_STD * std)
    width_20d_avg  = float(widths_series.rolling(20).mean().iloc[-1])
    width_ratio    = round(bb_range / width_20d_avg, 3) if width_20d_avg > 0 else 1.0
...
def compute_volume_trend(df: pd.DataFrame) -> dict:
    ...
    Also detects divergence between volume and price direction:
    ...
    # Price direction over same lookback window
    recent_closes  = df["Close"].tail(VOLUME_LOOKBACK)
    price_rising   = float(recent_closes.iloc[-1]) > float(recent_closes.iloc[0])
    price_falling  = float(recent_closes.iloc[-1]) < float(recent_closes.iloc[0])

    # Volume/price divergence detection
    low_volume  = ratio < VOLUME_FALLING
    high_volume = ratio > VOLUME_RISING

    if low_volume and price_rising:
        signal = "low volume rally -- weak conviction, potential false breakout"
        icon   = "⚠️"
        divergence = "low_volume_rally"
    elif high_volume and price_falling:
        signal = "high volume selloff -- distribution, bearish confirmation"
        icon   = "⚠️"
        divergence = "high_volume_selloff"
    elif high_volume and price_rising:
        signal = "rising -- confirms price move"
        icon   = "✅"
        divergence = None
    elif low_volume:
        signal = "falling -- weak conviction"
        icon   = "⚠️"
        divergence = None
    else:
        signal = "stable"
